...
//    }
    ...
    public int update(user User) throws SQLException{
        PreparedStatement ps = cnxDB.getCon().prepareStatement("UPDATE `user` SET `nom`=? ,`prenom`=?,`email`=?,`datenais`=?,`lieunais`=?,`Telephone`=?,`sexe`=?,`photo`=? WHERE id_user=?");
        ps.setString(1, User.getNom());
        ps.setString(2, User.getPrenom());
        ps.setString(3, User.getEmail());
        ps.setString(4, User.getDateNais());
    ...
    public int update(enseignant tsh) throws SQLException{
    ...
    public static void main (String []args){
        ...
        System.out.println(tsh.getAllStudentsByTeacher(3));
    }
}
