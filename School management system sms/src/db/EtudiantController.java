...
public class EtudiantController {
    ...
    public ObservableList<etudiant> getAllStudentsByClasseAndSeance(int id_Classe , int id_Seance){
        ObservableList<etudiant> List = FXCollections.observableArrayList();
        try {
            ...
            "    LEFT JOIN seance ON seancegenerique.id_seancegenerique= seance.id_seancegenerique\n" +
            "    LEFT JOIN `cycle` ON `classegenerique`.`id_cycle` = `cycle`.`id_cycle`\n" +
            "    LEFT JOIN `niveau` ON `classegenerique`.`id_niveau` = `niveau`.`id_niveau`\n" +
            "WHERE `classe`.`id_classe` = ? and seance.id_seance=?");
            ps.setInt(1, id_Classe);
